...
public class Debug extends DefaultTask {
     ...
     */
    public Debug() {
        target = getProject().getObjects().property(JavaForkOptions.class);
        port = getProject().getObjects().property(Integer.class);
        server = getProject().getObjects().property(Boolean.class);
        suspend = getProject().getObjects().property(Boolean.class);
        finalizedBy(target);
    }

    @TaskAction
    private void run() {
        target.get().debugOptions(options -> {
